 ...
 *
 *         Files with a pwg extension describe a set of pairwise relations.
 *         Every possible pairwise comparison is listed (if it appears in the
 *         data set) along with the number of time it appears. For example: 61
 *         A,B; 21 B,A.
 */

public final class EDFPwg extends AbstractEDFFile {

	private final String TABLENAME = "Pairwise Graph";

	public EDFPwg(URL url, String rowPrefix, ExecutionContext exec) {
		super(url, rowPrefix, exec);
	}

	@Override
	public boolean isRightGraph() {
		return getGraph().isPairwise();
	}
	...
	public DataCell[] readDataFile() throws IllegalStateException, InvalidSettingsException {
		...
		boolean fueInsertada = getGraph().insertEdge(dataList.get(1), dataList.get(2), dataList.get(0));
		if (!fueInsertada) {
