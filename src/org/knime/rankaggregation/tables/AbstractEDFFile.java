...
public abstract class AbstractEDFFile extends RowIterator implements DataTable {

	private final NodeLogger LOGGER = NodeLogger.getLogger(AbstractEDFFile.class);
	private DataTableSpec table;
	private EDFGraph graph;
	private URL url;
	...
	public AbstractEDFFile(URL url, String rowPrefix, ExecutionContext exec) {
		if (url == null) {
	 ...
	 * @return boolean
	 */
	protected abstract boolean isRightGraph();
	...
	private void readHeaderFile() throws CanceledExecutionException, InvalidSettingsException {
		HashMap<Integer, String> candidatesName = new HashMap<Integer, String>();
		...
		while (!isEnd) {
			...
			for (int i = 1; i <= NumberOfCandidates; i++) {
				...
				if (!candidatesName.containsValue(candidateName)) {
					candidatesName.put(key, candidateName);
				} else
					throw new InvalidSettingsException(
	...
	private void createGraph() {
		...
		int numColumns = this.table.getNumColumns() - 1;
		for (int i = 0; i < numColumns; i++) {
			String label = getDataTableSpec().getColumnSpec(i).getName();
			labels.add(label);
		}
		this.graph = new EDFGraph(numColumns, labels);
	}

	public void fillDataTable() throws Exception {
